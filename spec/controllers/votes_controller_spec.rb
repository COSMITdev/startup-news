  ...
  describe "#create" do
    context "successfull new vote" do
      ...
      context "guest user" do
        before { controller.request.stub(:ip).and_return("127.0.0.1") }

        context "up vote" do
          it "calls Vote.find_user_for_news and returns false" do
            Vote.should_receive(:find_ip_for_news)
            post(:create, { id: News.make!, vote: "up" })
          end
          ...
          it "assigns current_user to vote.user" do
            post(:create, { id: News.make!, vote: "up" })
            expect(assigns(:vote).user).to be_nil
          end
          ...
          it "assigns true to vote.is_up" do
            post(:create, { id: News.make!, vote: "up" })
            expect(assigns(:vote).is_up).to be_true
          end
        end
    ...
    context "successfull change of vote" do
      ...
      context "guest user" do
        ...
        context "change up to down" do
          before { @vote = Vote.make!(is_up: true, ip: "127.0.0.1") }
          ...
          it "Vote.find_ip_for_news returns true" do
            Vote.should_receive(:find_ip_for_news).with("127.0.0.1", @vote.news)
            post(:create, { id: @vote.news.to_param, vote: "down" })
          end
        ...
        context "change down to up" do
          before { @vote = Vote.make!(is_up: false, ip: "127.0.0.1") }
